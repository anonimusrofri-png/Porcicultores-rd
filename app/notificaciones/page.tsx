          ...
          {notificaciones.map((n) => {
            ...
            return (
              <div key={n.id} style={{ backgroundColor: bg, borderRadius: '16px', padding: '20px', border: `1px solid ${color}20` }}>
                ...
                {n.descripcion && <p style={{ color: '#64748b', fontSize: '13px' }}>{n.descripcion}</p>}
                {n.estado === 'aprobada' && (
                  <Link href="/marketplace" style={{ display: 'inline-block', marginTop: '12px', backgroundColor: '#0a2463', color: 'white', padding: '8px 16px', borderRadius: '8px', textDecoration: 'none', fontSize: '13px', fontWeight: '700' }}>
                    Ver en Marketplace
                  </Link>
                )}
              </div>
