...
export default function Login() {
  ...
  const handleGoogle = async () => {
    await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: 'https://porcicultores-rd.vercel.app/' } })
  }
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f0f4f8', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '40px', width: '100%', maxWidth: '420px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ display: 'flex', marginBottom: '24px', backgroundColor: '#f0f4f8', borderRadius: '12px', padding: '4px' }}>
          <div style={{ flex: 1, textAlign: 'center', padding: '10px', backgroundColor: 'white', borderRadius: '10px', color: '#0a2463', fontWeight: '700', fontSize: '14px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>Iniciar Sesion</div>
          <Link href="/registro" style={{ flex: 1, textAlign: 'center', padding: '10px', color: '#64748b', fontWeight: '600', fontSize: '14px', textDecoration: 'none' }}>Registrarse</Link>
        </div>
        <button onClick={handleGoogle} style={{ width: '100%', padding: '12px', backgroundColor: 'white', color: '#1e293b', border: '1px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', fontSize: '14px', fontWeight: '700', marginBottom: '16px' }}>
        ...
        {error && <div style={{ backgroundColor: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '10px', padding: '12px', marginBottom: '16px', color: '#dc2626', fontSize: '13px', fontWeight: '600' }}>{error}</div>}
        <button onClick={handleLogin} disabled={cargando} style={{ width: '100%', padding: '14px', backgroundColor: '#0a2463', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '16px', fontWeight: '800', marginBottom: '16px' }}>
          {cargando ? 'Entrando...' : 'Iniciar Sesion'}
        </button>
        <div style={{ textAlign: 'center', backgroundColor: '#f0f4f8', borderRadius: '12px', padding: '16px' }}>
          <p style={{ color: '#64748b', fontSize: '13px', marginBottom: '8px' }}>No tienes cuenta?</p>
